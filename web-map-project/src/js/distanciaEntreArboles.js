...
document.getElementById('distancia-arboles').addEventListener('click', function() {
    fetch('js/Arbolado_Urbano.zip') // Assuming the shapefile is zipped
            ...
            shp(data).then(function(geojson) {
                ...
                for (let i = 0; i < filteredFeatures.length; i++) {
                    for (let j = i + 1; j < filteredFeatures.length; j++) {
                        ...
                        distances.push({
                            tree1: filteredFeatures[i].properties.name,
                            tree2: filteredFeatures[j].properties.name,
                ...
                distances.forEach((d, index) => {
                    doc.text(`${index + 1}. ${d.tree1} - ${d.tree2}: ${d.distance}`, 10, 20 + index * 10);
                });
                console.log('Generating PDF...'); // Debugging line
                doc.save('distancia_entre_arboles.pdf');
            }).catch(error => console.error('Error converting SHP to GeoJSON:', error));
        })
        .catch(error => console.error('Error loading SHP:', error));
});
